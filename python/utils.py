...
def train(word2vec, dataset, parameters):
    ...
    with tf.device(device_string):
        ...
        dev_summary_writer = tf.summary.FileWriter(logdir_dev)
...
def test(word2vec, dataset, parameters, loadpath):
    ...
    with tf.device(device_string):
        ...
        batcher = Batcher(word2vec=word2vec, settings=parameters["settings"])
        # train_batches = batcher.batch_generator(dataset=dataset["train"], num_epochs=parameters["num_epochs"],
        ...
        #     if train_step % 100 == 0:
        #         sys.stdout.write(
        #             "\rTRAIN | epoch={0}/{1}, step={2}/{3} | loss={4:.2f}, accuracy={5:.2f}%   ".format(epoch + 1,
        #                                                                                                 parameters[
        #                                                                                                     "num_epochs"],
        #                                                                                                 train_step % num_step_by_epoch,
        #                                                                                                 num_step_by_epoch,
        #                                                                                                 train_loss,
        #                                                                                                 100. * train_accuracy))
        #         sys.stdout.flush()
        ...
        #             }
        #
        #             summary_str, test_loss, test_accuracy = sess.run([test_summary_op, loss, accuracy],
        #                                                              feed_dict=feed_dict)
        #             print"\nTEST | loss={0:.2f}, accuracy={1:.2f}%   ".format(test_loss, 100. * test_accuracy)
        #             print ""
        #             test_summary_writer.add_summary(summary_str, train_step)
